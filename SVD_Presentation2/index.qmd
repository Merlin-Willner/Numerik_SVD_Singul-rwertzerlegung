...
![](assets/generated/svd_building_block_example.svg){.svd-block-example fig-alt="Grafisches Beispiel fuer mehrere SVD-Bildbausteine und ihre ueberlagerte Summe"}
...
::: {.svd-single-note}
$u_1v_1^T$ legt die **Form** fest. $\sigma_1$ legt die **Stärke** fest.
...
::: {.derivation-fullnote}
Die Höhenantwort $A{\color{#16a34a}{v}}$ zeigt, wie stark das Spaltenmuster in jeder Bildzeile vorkommt. Ihre Länge macht daraus eine Gesamtstärke:
...
Wir suchen das normierte Spaltenmuster, das die stärkste Höhenantwort erzeugt. Im Beispiel verwenden wir 0/1-Werte. Für die eigentliche Suche normieren wir $v$ auf $\|v\|=1$, damit alle Spaltenmuster gleich lang sind und $\|Av\|$ nur die Stärke des Musters im Bild misst -- nicht eine künstliche Skalierung von $v$.
...
Die Spalten von $Q$ sind orthogonale Richtungen. Die zugehörigen Eigenwerte $\lambda_i$ können wir später als Stärken interpretieren.
...
::: {.uebergang}
**Können wir aus \(A\) eine symmetrische Matrix bauen, die dieselbe Stärkefrage für Spaltenmuster beantwortet?**
:::

:::

## Warum führt die Stärkemessung zu $A^TA$?
...
Da $\|A{\color{#16a34a}{v}}\|\ge0$, dürfen wir stattdessen die quadrierte Stärke maximieren:
...
![](assets/generated/ata_strength_derivation.svg){.ata-strength-derivation fig-alt="Die quadrierte Stärke eines Spaltenmusters führt zur Bewertungsmatrix A transponiert A"}
:::

::: {.uebergang}
Für ein festes Spaltenmuster ${\color{#16a34a}{v}}$ ist ${\color{#16a34a}{v^T}}A^TA{\color{#16a34a}{v}}$ genau dieselbe Zahl wie $\|A{\color{#16a34a}{v}}\|^2$: die quadrierte Stärke dieses Musters im Originalbild.
...
Für solche Matrizen liefert der Spektralsatz orthogonale Richtungen.
:::
...
::: {.uebergang}
**Nächste Frage: Welche normierten Spaltenmuster ${\color{#16a34a}{v_i}}$ maximieren diese Bewertung? Dafür wenden wir jetzt den Spektralsatz auf $A^TA$ an.**
:::
...
2. **Sortierbare Bewertungen**

$$
\lambda_1\ge\lambda_2\ge\dots\ge0
$$

Die ersten ${\color{#16a34a}{v_i}}$ gehören zu den größten Bewertungen.
:::
...
::: {.uebergang}
**Wir haben jetzt unabhängige Spaltenmuster ${\color{#16a34a}{v_i}}$ und zugehörige Bewertungen $\lambda_i$. Nächste Frage: Warum ist $\lambda_i$ die quadrierte Stärke des Musters ${\color{#16a34a}{v_i}}$?**
:::

:::

## Warum sind die Eigenwerte die quadrierte Stärke?
...
Jetzt prüfen wir, wie stark so ein gefundenes Muster im Originalbild $A$ vorkommt.

Für ein gefundenes Spaltenmuster ${\color{#16a34a}{v_i}}$ messen wir wieder die Stärke der Höhenantwort:
...
Damit ist $\lambda_i$ die **quadrierte Stärke** des Musters im Originalbild.
...
::: {.eigenvalue-graphic}
![](assets/generated/eigenvalue_to_singular_value.svg){.eigenvalue-singularvalue fig-alt="Eigenwert als quadrierte Stärke und Singulärwert als tatsächliche Stärke eines Spaltenmusters"}
:::

::: {.uebergang}
**Merke: Eigenwert = quadrierte Stärke, Singulärwert = tatsächliche Stärke. Als Nächstes fehlt noch die Höhenverteilung ${\color{#1e88ff}{u_i}}$.**
:::
...
- ${\color{#f2aa00}{\sigma_i}}$: Stärke dieses Musters
...
Also enthält $A{\color{#16a34a}{v_i}}$ Form und Stärke. Für die SVD trennen wir beides:
...
Durch das Teilen durch ${\color{#f2aa00}{\sigma_i}}$ bleibt die Form über die Höhe erhalten; die Stärke wird entfernt.
:::
...
::: {.uebergang}
**Merke: $A{\color{#16a34a}{v_i}}$ enthält Form und Stärke. Durch ${\color{#1e88ff}{u_i}}=\frac{A{\color{#16a34a}{v_i}}}{\sigma_i}$ erhalten wir die normierte Höhenverteilung.**
:::
...
Das heißt: Die Höhenantwort des Spaltenmusters besteht aus **Stärke** mal **normierter Höhenverteilung**.
:::
...
- ${\color{#1e88ff}{u_i}}$: Verteilung über die Bildzeilen
- ${\color{#f2aa00}{\sigma_i}}$: Stärke des Bausteins

Das äußere Produkt ${\color{#1e88ff}{u_i}}{\color{#16a34a}{v_i^T}}$ macht daraus eine 2D-Bildschicht.
:::

::: {.svd-block-from-parts-wrap}
![](assets/generated/svd_block_from_parts.svg){.svd-block-from-parts fig-alt="Aus Spaltenmuster, Höhenverteilung und Stärke entsteht ein SVD-Bildbaustein"}
:::
...
::: {.derivation-fullnote}
**Merke: Die SVD ist die kompakte Schreibweise für die Summe aller Bildbausteine. Reduziert heißt hier noch nicht komprimiert, sondern nur: Null-Bausteine wurden weggelassen. Kompression beginnt erst, wenn wir zusätzlich kleine, aber nicht-null Bausteine weglassen.**
:::
...
::: {.derivation-right}
Die Stärke eines Bausteins steckt in ${\color{#f2aa00}{\sigma_i}}$.

Weil die SVD-Bausteine orthogonal zueinander sind, addieren sich ihre quadratischen Stärken sauber:
...
::: {.derivation-fullnote}
**Merke: $A_k$ behält die $k$ stärksten Bildbausteine. Der Fehler besteht aus den weggelassenen schwachen Bausteinen. Weil die ${\color{#f2aa00}{\sigma_i}}$ absteigend sortiert sind, ist das Abschneiden bei $k$ die beste Rang-$k$-Näherung.**
:::
...
Stärke × Höhenverteilung × Spaltenmuster
:::
