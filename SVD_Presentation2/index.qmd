...
::: {.svd-single-meta}
Für das Prinzip verwenden wir hier den kleinen Baustein aus der vorherigen Folie. Beim echten Bild hätten $u_i$ und $v_i$ entsprechend viele Einträge wie Bildzeilen und Bildspalten.
:::

![](assets/generated/svd_single_block_decomposition.svg){.svd-single-block fig-alt="Zerlegung eines einzelnen SVD-Bausteins in u1, v1 transponiert und sigma1"}
...
::: {.uebergang}
Damit liefert der Spektralsatz auf $A^TA$ **genau die zwei Eigenschaften aus den vorherigen Folien**: orthogonale, also **unabhängige** Muster, die sich nach Eigenwert **sortieren** lassen — die stärksten zuerst.
:::
