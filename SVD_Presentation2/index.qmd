...
## Beispiel: SVD einer \(3\times3\)-Matrix

::: {.worked-svd-slide}

::: {.worked-svd-input .worked-svd-block}
**Gegeben**
...
::: {.worked-svd-step}
**2. Eigenwerte und rechte Richtungen**

$$
A^TA\text{ ist diagonal}
\Rightarrow
\lambda=2,8,9.
$$
...
$$
(\lambda_1,v_1)=(9,e_3),\quad
(\lambda_2,v_2)=(8,e_2),\quad
(\lambda_3,v_3)=(2,e_1).
$$
:::
...
\sigma_1=3,\quad
\sigma_2=2\sqrt2,\quad
\sigma_3=\sqrt2
$$
:::

::: {.worked-svd-step .worked-svd-step-4}
**4. Linke Richtungen aus \(u_i=\frac{Av_i}{\sigma_i}\)**

Jedes gefundene Spaltenmuster wird mit \(A\) multipliziert. Danach trennen wir die Länge \(\sigma_i\) von der normierten Richtung \(u_i\).

$$
\begin{aligned}
v_1=e_3:\quad
Av_1&=(0,0,3)^T=3(0,0,1)^T
&\Rightarrow\quad u_1&=(0,0,1)^T\\
v_2=e_2:\quad
Av_2&=(2,-2,0)^T=2\sqrt2\,\frac1{\sqrt2}(1,-1,0)^T
&\Rightarrow\quad u_2&=\frac1{\sqrt2}(1,-1,0)^T\\
v_3=e_1:\quad
Av_3&=(1,1,0)^T=\sqrt2\,\frac1{\sqrt2}(1,1,0)^T
&\Rightarrow\quad u_3&=\frac1{\sqrt2}(1,1,0)^T
\end{aligned}
$$
:::
...
::: {.worked-svd-factor .sigma-factor}
**\(\Sigma\)**
...
**Ergebnis:**
$A=U\Sigma V^T
=3u_1v_1^T+2\sqrt2\,u_2v_2^T+\sqrt2\,u_3v_3^T$.
:::
...
::: {.derivation-slide .two-column-derivation .building-block-layout}
...
::: {.derivation-slide .two-column-derivation .orthogonal-layout}
...
$\Rightarrow$ Für Kompression wollen wir aber, dass jeder gespeicherte Baustein wirklich neue Information bringt.
:::
...
::: {.derivation-slide .two-column-derivation .strong-pattern-layout}
...
::: {.derivation-fullnote}
Die Höhenantwort $A{\color{#16a34a}{v}}$ zeigt, wie stark das Spaltenmuster in jeder Bildzeile vorkommt. Ihre Länge misst die Gesamtstärke: $\|A{\color{#16a34a}{v}}\|=\sqrt{2^2+2^2+2^2+2^2+0^2+0^2}=4$.

Je größer $\|A{\color{#16a34a}{v}}\|$, desto stärker ist die Antwort auf das Muster ${\color{#16a34a}{v}}$. Damit wird unsere Suche konkret: $\max_{\|{\color{#16a34a}{v}}\|=1}\|A{\color{#16a34a}{v}}\|$.
...
$A{\color{#16a34a}{v}}=\text{Höhenantwort}$ und $\|A{\color{#16a34a}{v}}\|=\text{Gesamtstärke}$.
:::
...
::: {.derivation-slide .two-column-derivation .ata-solution-layout}
...
$\Rightarrow$ Für solche Matrizen liefert der Spektralsatz orthogonale Eigenrichtungen.
:::
...
::: {.uebergang}
Damit liefert der Spektralsatz auf $A^TA$ **genau die zwei Eigenschaften von Folie 19/20**: orthogonale, also **unabhängige** Muster, die sich nach Eigenwert **sortieren** lassen — die stärksten zuerst.
:::
...
::: {.derivation-left}
Aus Spaltenmuster, Höhenverteilung und Singulärwert entsteht ein einzelner Bildbaustein:
...
Dabei ist ${\color{#16a34a}{v_i^T}}$ das Spaltenmuster, ${\color{#1e88ff}{u_i}}$ die Höhenverteilung und ${\color{#f2aa00}{\sigma_i}}$ die Stärke dieses Bausteins.
